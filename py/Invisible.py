...
fname = "data/Invisible.txt"
dataf = open(fname, "r")
a, b, m1, z, g = [float(x) for x in dataf.readline().split()]
dataf.close()
a = int(a)
b = int(b)

dts = ["Eb", "SM", "21", "31", "32", "All"]
dt = [(d, "f") for d in dts]
...
for i in range(1, len(dts)):
	d = dts[i]
	try:
		label = r"$g^{(\prime)}_{%i}$" % int(d)
	except:
		label = r"${\rm %s}$" % d
	ls = "-"
	if d == "32": ls = "--"
	plt.plot(data["Eb"], data[d], label = label, ls = ls)
...
plt.ylabel(r"$\bar P_{%s %s}^{\rm inv}$" % (flavor_texs[a], flavor_texs[b]))
